    ...
    <div className="glass-stat p-5">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className={`text-3xl font-semibold mt-1 ${color || 'text-white/90'}`}>{value}</p>
        </div>
        ...
        {stats.recent_activity.map((item) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className="text-xs text-white/30">{timeAgo(item.date)}</span>
            </div>
          </Link>
        ))}
      </div>
...
export default async function DashboardPage() {
  ...
  return (
    <AppShell>
      <div className="p-8 glass-page-bg min-h-screen">
        <PageHeader
          title="Dashboard"
          subtitle={greeting}
          actions={
